<template>
  ...
  <el-form-item v-if="timerType === 'timeDate'" label="指定时间">
    <el-date-picker type="datetime" v-model="timeDate" value-format="YYYY-MM-DDTHH:mm:ssZ" style="width: 100%" />
  </el-form-item>
  ...
  <el-form-item v-if="timerType === 'timeCycle'" label="结束时间">
    <el-date-picker type="datetime" v-model="timeCycleEndDate" value-format="YYYY-MM-DDTHH:mm:ssZ" style="width: 100%"
      clearable />
  </el-form-item>
</template>
...
const timerType = computed<TimerType | undefined>({
  get() {
    const definition = bpmnSelectedElem.value?.businessObject?.eventDefinitions?.[0]
