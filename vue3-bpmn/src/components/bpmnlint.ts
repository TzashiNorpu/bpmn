...
class WorkflowOutcomeValidator extends BpmnLintBase {
  ...
  validate(): boolean {
    const userTasks = this.registry
      .filter((it) => it.type === 'bpmn:UserTask')
      .filter((it) => it.outgoing?.length > 1)
    for (const userTask of userTasks) {
      for (const connection of userTask.outgoing) {
...
class MultiUserTaskValidator extends BpmnLintBase {
  ...
  validate(): boolean {
    const userTasks = this.registry
      .filter((it) => it.type === 'bpmn:UserTask')
      .filter((it) => !!it.businessObject.loopCharacteristics)
    if (userTasks?.length) {
